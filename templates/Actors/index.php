...
<div id="kt_content_container" class="container-xxl">
    ...
    <!--begin::Card header-->        
        <h3 class="card-header border-0 pt-6"><?= __('Actors') ?></h3>
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-2" id="kt_subscriptions_table">
                <thead>
                    <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <th><?= $this->Paginator->sort('disability_id') ?></th>
                        ...
                        <th><?= $this->Paginator->sort('production_scale_id') ?></th>
                        <th><?= $this->Paginator->sort('other_related_activity') ?></th>
                        <th><?= $this->Paginator->sort('additional_comments') ?></th>
                ...
                <tbody class="text-gray-600 fw-bold">
                    ...
                    <tr>
                        ...
                        <td><?= $actor->has('sex') ? $this->Html->link($actor->sex->name, ['controller' => 'Sexes', 'action' => 'view', $actor->sex->id]) : '' ?></td>
                        <td><?= $actor->has('disability') ? $this->Html->link($actor->disability->name, ['controller' => 'Disabilities', 'action' => 'view', $actor->disability->id]) : '' ?></td>
                        <td><?= $actor->has('sector') ? $this->Html->link($actor->sector->name, ['controller' => 'Sectors', 'action' => 'view', $actor->sector->id]) : '' ?></td>
                        <td><?= h($actor->phone) ?></td>
                        <td><?= h($actor->other_phone) ?></td>
                        <td><?= $actor->has('community') ? $this->Html->link($actor->community->name, ['controller' => 'Communities', 'action' => 'view', $actor->community->id]) : '' ?></td>
                        <td><?= h($actor->suburb) ?></td>
                        <td><?= h($actor->registed_with_RGD) ?></td>
                        <td><?= h($actor->registed_with_Assembly) ?></td>
                        <td><?= $actor->has('production_scale') ? $this->Html->link($actor->production_scale->name, ['controller' => 'ProductionScales', 'action' => 'view', $actor->production_scale->id]) : '' ?></td>
                        <td><?= h($actor->other_related_activity) ?></td>
                        <td><?= h($actor->additional_comments) ?></td>
                        ...
                        <td class="text-end">
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-2" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3">
                                    <?= $this->Html->link(__('View'), ['action' => 'view', $actor->id],['class'=>'menu-link px-3']) ?>
                                </div>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                <?php if($loggedin_user_role==="admin" || $loggedin_user_role==="super" ): ?>    
                                <div class="menu-item px-3">
                                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $actor->id],['class'=>'menu-link px-3']) ?>
                                </div>
                                <?php endif?>
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                ...
                                <div class="menu-item badge-light-danger px-3   ">
                                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $actor->id], ['class'=>'menu-link px-3','confirm' => __('Are you sure you want to delete # {0}?', $actor->id)]) ?>
                                </div>
                                <?php endif?>
                                <!--end::Menu item-->
                            </div>
